...
if __name__ == '__main__':
    ...
    model = CircuitLSTMDecoderw2Q(
        num_ancillas, num_ancillas * 8, num_layers=2,
        if_large_lstm_2q=args.if_large_lstm_2q).to(device)

    old_model = torch.load(os.path.join(args.load_model, f'model_{args.run_index}.pt'), weights_only=False)
